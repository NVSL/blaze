...
#ifndef GALOIS_LARGEPMEMARRAY_H
#define GALOIS_LARGEPMEMARRAY_H
...
protected:
  ...
  void allocate(size_type n, AllocType t) {
    ...
    switch (t) {
    case Blocked:
      galois::gDebug("Block-alloc'd (PMEM)");
      m_realdata =
          substrate::largeMallocBlocked(n * sizeof(T), runtime::activeThreads, true);
      break;
    case Interleaved:
      galois::gDebug("Interleave-alloc'd (PMEM)");
      m_realdata = substrate::largeMallocInterleaved(n * sizeof(T),
                                                     runtime::activeThreads, true);
      break;
    case Local:
      galois::gDebug("Local-allocd (PMEM)");
      m_realdata = substrate::largeMallocLocal(n * sizeof(T), true);
      break;
    case Floating:
      galois::gDebug("Floating-alloc'd (PMEM)");
      m_realdata = substrate::largeMallocFloating(n * sizeof(T), true);
      break;
    };
...
public:
  /**
   * Wraps existing buffer in LargePmemArray interface.
   */
  LargePmemArray(void* d, size_t s) : m_data(reinterpret_cast<T*>(d)), m_size(s) {}

  LargePmemArray() : m_data(0), m_size(0) {}

  LargePmemArray(LargePmemArray&& o) : m_data(0), m_size(0) {
    std::swap(this->m_realdata, o.m_realdata);
    std::swap(this->m_data, o.m_data);
    std::swap(this->m_size, o.m_size);
  }

  LargePmemArray& operator=(LargePmemArray&& o) {
    std::swap(this->m_realdata, o.m_realdata);
    std::swap(this->m_data, o.m_data);
    std::swap(this->m_size, o.m_size);
    return *this;
  ...
  LargePmemArray(const LargePmemArray&) = delete;
  LargePmemArray& operator=(const LargePmemArray&) = delete;
  ...
  friend void swap(LargePmemArray& lhs, LargePmemArray& rhs) {
...
public:
  LargePmemArray(void* d, size_t s) {}
  LargePmemArray()                  = default;
  LargePmemArray(const LargePmemArray&) = delete;
  LargePmemArray& operator=(const LargePmemArray&) = delete;

  friend void swap(LargePmemArray&, LargePmemArray&) {}
