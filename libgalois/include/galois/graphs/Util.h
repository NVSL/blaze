...
template <typename GraphTy>
void readGraphDispatch(GraphTy& graph, read_mmap_graph_tag tag,
                       const std::string& filename) {
  FileGraph f;
  f.setIgnoreUnmapping(true);
  f.fromFileInterleaved<typename GraphTy::file_edge_data_type>(filename);
  readGraphDispatch(graph, tag, f);
}

template <typename GraphTy>
void readGraphDispatch(GraphTy& graph, read_mmap_part_graph_tag tag,
                       const std::string& filename) {
  graph.readFrom(filename);
}
...
template <typename GraphTy>
void readGraphDispatch(GraphTy& graph, read_mmap_graph_tag, FileGraph& f) {
  graph.allocateFrom(f);

  ReadGraphConstructFrom<GraphTy> reader(graph, f);
  galois::on_each(reader);
  graph.mapEdgesFrom(f);
}
 ...
 */
template <typename GraphTy>
struct ReadGraphConstructFromPartitions {
  GraphTy& graph;
  std::vector<FileGraph>& f;
  ReadGraphConstructFromPartitions(GraphTy& g, std::vector<FileGraph>& _f) : graph(g), f(_f) {}
  void operator()(unsigned tid, unsigned total) {
    graph.constructFrom(f, tid, total);
  }
};
...
template <typename GraphTy>
void readGraphPartitions(GraphTy& graph, const std::string& filename, unsigned int numParts) {
  // Read to FileGraph
  std::vector<FileGraph> f;
  for (unsigned int i = 0; i < numParts; i++) {
    f.emplace_back();
    std::ostringstream partname;
    partname << filename << "." << i << ".of." << numParts;
    f[i].fromFileInterleaved<typename GraphTy::file_edge_data_type>(partname.str());
  }

  // Allocate memory for the target graph
  graph.allocateFrom(f);
  ReadGraphConstructFromPartitions<GraphTy> reader(graph, f);
  galois::on_each(reader);
}
