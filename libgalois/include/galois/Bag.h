...
public:
  ...
  local_iterator local_begin(unsigned tid) {
    return local_iterator(&heads, tid);
  }
  local_iterator local_end(unsigned tid) {
    return local_iterator(&heads, tid + 1);
  }
...
public:
  ...
  bool empty() {
    return std::all_of(bags.begin(), bags.end(), [](InsertBag<T>& bag){ return bag.empty(); } );
  }
  ...
  void swap(InsertBags& o) {
    for (int i = 0; i < bags.size(); i++) {
      bags[i].swap(o[i]);
    }
  }

  InsertBag<T>& operator[](int i) {
    return bags[i];
  }
};
