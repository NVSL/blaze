...
/**
 * Specify the prefetch distance for loops that support prefetching
 * Currently, only runtime argument, e.g., galois::prefetch_distance(8), is supported.
 */
struct prefetch1_distance_tag {
  enum { MIN = 0, MAX = 4096 };
};
struct prefetch1_distance : public trait_has_value<unsigned>, prefetch1_distance_tag {
  unsigned regulate(const unsigned pd) const {
    return std::min(std::max(unsigned(prefetch1_distance_tag::MIN), pd),
                    unsigned(prefetch1_distance_tag::MAX));
  }
  prefetch1_distance(unsigned pd = 0) : trait_has_value(regulate(pd)) {}
};

struct prefetch2_distance_tag {
  enum { MIN = 0, MAX = 4096 };
};
struct prefetch2_distance : public trait_has_value<unsigned>, prefetch2_distance_tag {
  unsigned regulate(const unsigned pd) const {
    return std::min(std::max(unsigned(prefetch2_distance_tag::MIN), pd),
                    unsigned(prefetch2_distance_tag::MAX));
  }
  prefetch2_distance(unsigned pd = 0) : trait_has_value(regulate(pd)) {}
};
