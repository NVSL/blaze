...
void ThreadPool::forkWithMain(unsigned num, std::vector<std::function<void(void)>>& f) {
  GALOIS_ASSERT(num <= mi.maxThreads, "Invalid tid range");
...
void ThreadPool::fork(unsigned begin, std::function<void(void)>& f) {
  unsigned end  = begin + 1;
  GALOIS_ASSERT(begin > 0 && end <= mi.maxThreads, "Invalid tid range");
  for (unsigned i = begin; i < end; i++) {
    works[i] = f;
  }
  auto child    = signals[begin];
  child->wbegin = begin + 1;
  child->wend   = end;
  child->wakeup(masterFastmode);
}

void ThreadPool::fork(unsigned begin, unsigned num, std::vector<std::function<void(void)>>& f) {
  unsigned end  = begin + num;
  GALOIS_ASSERT(begin > 0 && end <= mi.maxThreads, "Invalid tid range");
  GALOIS_ASSERT(f.size() == num, "Invalid arguments");
  for (unsigned i = begin, fi = 0; i < end; i++, fi++) {
    works[i] = f[fi];
  }
  auto child    = signals[begin];
  child->wbegin = begin + 1;
...
void ThreadPool::join(unsigned begin) {
  auto child    = signals[begin];
  while (!child->done) {
    asmPause();
  }
...
void ThreadPool::runDedicated(std::function<void(void)>& f) {
  ...
  unsigned tid  = mi.maxThreads - reserved;
  works[tid]    = [&f]() { throw dedicated_ty{f}; };
  auto child    = signals[tid];
  child->wbegin = 0;
  child->wend   = 0;
  child->done   = 0;
  child->wakeup(masterFastmode);
...
void ThreadPool::runDedicated(std::function<void(void)>&& f, unsigned tid) {
  GALOIS_ASSERT(!running,
                "can't start dedicated thread durring parallel section");
  GALOIS_ASSERT(tid < mi.maxThreads, "Invalid tid");
  works[tid]    = [&f]() { throw dedicated_ty{f}; };
  auto child    = signals[tid];
  child->wbegin = 0;
  child->wend   = 0;
