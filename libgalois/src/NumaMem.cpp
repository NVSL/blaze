...
LAptr galois::substrate::largeMallocInterleaved(size_t bytes,
  ...
  void* data;
  if (!pmem) data = allocPages(bytes / allocSize(), false);
  else data = allocPagesPmem(bytes / allocSize(), false);
...
LAptr galois::substrate::largeMallocLocal(size_t bytes, bool pmem) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a prefaulted allocation
  void* data;
  if (!pmem) data = allocPages(bytes / allocSize(), true);
  else data = allocPagesPmem(bytes / allocSize(), true);

  return LAptr{data, internal::largeFreer{bytes}};
}

LAptr galois::substrate::largeMallocFloating(size_t bytes, bool pmem) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a non-prefaulted allocation
  void* data;
...
LAptr galois::substrate::largeMallocBlocked(size_t bytes, unsigned numThreads, bool pmem) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a non-prefaulted allocation
  void* data;
